...
class Countdown extends React.Component {
    ...
    componentDidMount() {
        ...
        db.onceGetImagesForCalender(calId)
            .then(snapshot => {
                ...
                const coverPath =
                    'https://s3.eu-west-2.amazonaws.com/elasticbeanstalk-eu-west-2-431995029029/countdown_images/covers/' +
                    fireObj.coverUrl
                this.setState({
                    coverPath,
    ...
    timerRender = ({ days, hours, minutes, seconds, completed }) => {
        if (completed) {
        ...
        return (
            <div>
                <div className="cal-body">
                    {theDate < 20181201 ? (
                        <div className="message-cont">
                            <div className="message">
                                <h1>
                                    Timer to {this.state.username}'s advent
                                    calendar
                                </h1>
                                <Timer
                                    date={this.timerTarget}
                    ...
                    <div
                        className={`cal-container un-blur-me   ${this.state
                            .showMessage && 'blur-me'}`}
                    >
                        <div
                            className="cal-cover"
                        ...
                        {this.state.imgPathArray.map((el, index) => {
                            return (
                                <div
                                    ...
                                    key={index}
                                    className={`cal-box ${el.visible &&
                                        el.dateIsGood &&
                                        'show-cal-img'}`}
                                >
                                    {el.path && (
                                        <img
                                            src={`https://res.cloudinary.com/dcqi9fn2y/image/upload/w_300,h_300,c_fill/${el.path}`}
                                            alt=""
                                        />
                                    )}

                                    {(!el.visible && el.dateIsGood) ||
                                    (!el.visible && !el.dateIsGood) ? (
                                        <div
                                            className={`cal-number ${!el.visible &&
                                                !el.dateIsGood &&
                                                'bad-number'} `}
                                            onClick={() => {
                                                this.handleShowImg(index)
                                            }}
                                        >
                                            <p
                                                style={{
                    ...
                    {this.state.lightboxVisible && (
                        ...
                        >
                            <div className="lightbox-img">
                                ...
                                <img
                                    src={`https://res.cloudinary.com/dcqi9fn2y/image/upload/${this.state.lightboxPath}`}
                                    alt=""
                                />
                            </div>
                        </div>
